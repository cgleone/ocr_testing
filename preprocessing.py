...
def recursive_deskew(img, angle, deskew_angles):
    ...
    # cv2.imshow('axis aligned image attempt', new_image)
    # cv2.waitKey(0)
    if abs(new_angle) < 3:
...
def get_skew_angle(image, deskew_angles):
    ...
    #contour_angles.sort(reverse=True)
    top_5_c_angles = contour_angles[:5]
    ...
    print("Original skew angle detected by rectangle approximation: ", angle)
...
def get_text_rotation(img):
    ...
    print("Text angle after rectangle deskew, detected by OCR: ", angle)
